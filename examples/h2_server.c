...
static ssize_t stub_recv(KlH2Session *self, const void *data, size_t len) {
    (void)self; (void)data;
    /* A real implementation would parse HTTP/2 frames here */
    return (ssize_t)len;
}

static int stub_submit_response(KlH2Session *self, uint32_t stream_id,
...
static int stub_want_write(KlH2Session *self) {
    (void)self;
    return 0;
}

static int stub_flush(KlH2Session *self) {
    (void)self;
    return 0;
}

static int stub_shutdown(KlH2Session *self) {
    (void)self;
    return 0;
}

static void stub_destroy(KlH2Session *self) {
    StubH2Session *s = (StubH2Session *)self;
    kl_free(s->alloc, s, sizeof(*s));
}

static KlH2Session *stub_factory(KlAllocator *alloc,
                                  KlH2Callbacks *callbacks,
                                  void *user_data) {
