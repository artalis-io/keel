...
make test         # build and run all 822 unit tests
...
33. **udp** — Non-blocking UDP datagram socket over `KlEventCtx`: async per-datagram receive with source + local (dest) address via `IP_PKTINFO`, capped whole-datagram send queue (backpressure), source-pinned sends (`kl_udp_send_to_from`), and multicast/broadcast — any-source group join/leave (`kl_udp_multicast_join`/`leave`, interface by index) + `SO_BROADCAST`, `IP_MULTICAST_TTL`/`LOOP`/`IF` config knobs (mDNS/SSDP/discovery). Foundation for a future async DNS resolver and QUIC/HTTP-3 (see `docs/udp_design.md`, `docs/udp_multicast_design.md`)
...
| `KlUdpConfig` | `udp.h` | UDP config: ctx, family, bind_addr/port, recv_buf_size, max_send_queue, reuse_addr/port, recv_pktinfo, so_rcvbuf/so_sndbuf, broadcast, multicast_ttl/disable_loop/iface/group, allocator |
