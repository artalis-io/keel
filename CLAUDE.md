...
make test         # build and run all 832 unit tests
...
33. **udp** — Non-blocking UDP datagram socket over `KlEventCtx`: async per-datagram receive with source + local (dest) address via `IP_PKTINFO`, capped whole-datagram send queue (backpressure), source-pinned sends (`kl_udp_send_to_from`), multicast/broadcast — any-source group join/leave (`kl_udp_multicast_join`/`leave`, interface by index) + `SO_BROADCAST`, `IP_MULTICAST_TTL`/`LOOP`/`IF` config knobs (mDNS/SSDP/discovery) — transparent `recvmmsg`/`sendmmsg` batching (Linux; `mmsg_batch` knob), and UDP GSO/GRO segmentation offload (`kl_udp_send_gso`, `recv_gro` with transparent split or `kl_udp_recv_segments` coalesced callback) — all Linux-only with per-datagram/plain fallbacks elsewhere. Foundation for a future async DNS resolver and QUIC/HTTP-3 (see `docs/udp_design.md`, `docs/udp_multicast_design.md`, `docs/udp_batching_design.md`)
34. **udp_server** — Datagram dispatch surface over `udp`, symmetric with `KlServer` (bind + per-datagram handler + reply). Shares a `KlEventCtx` so one process serves TCP HTTP and UDP on a single loop; horizontal scaling via `SO_REUSEPORT`. Passes through the multicast/broadcast config knobs + `kl_udp_server_multicast_join`/`leave` for discovery services (mDNS/SSDP)
35. **dns_resolver** — Built-in async DNS resolver over `udp` implementing the `KlResolver` vtable: non-blocking, dual-family A+AAAA queries issued concurrently (RFC 8305) with per-family timeout/retransmit, a Resolution-Delay cap (§3) so a slow/absent family never stalls, and a family-interleaved multi-address result (§4, preferred family first); plus `/etc/hosts` lookup, EDNS0, multiple nameservers with failover, and `resolv.conf` `search`/`ndots` expansion — replacing the blocking `getaddrinfo` fallback. Bounds-safe response parsers (`kl_dns_parse_response`, `kl_dns_parse_all`) are fuzzed (`fuzz_dns`)
...
| `KlUdpConfig` | `udp.h` | UDP config: ctx, family, bind_addr/port, recv_buf_size, max_send_queue, reuse_addr/port, recv_pktinfo, so_rcvbuf/so_sndbuf, mmsg_batch, recv_gro, broadcast, multicast_ttl/disable_loop/iface/group, allocator |
