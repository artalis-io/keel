/*
 * platform_thread_win.c: the Win32 side of the platform_thread.h seam. See that header for why the seam exists and why
 * it is this small.
 *
 ...
 * carries the real function and argument, heap-allocated so it outlives kl_plat_thread_create's frame and
 * freed by the thread itself. */
typedef struct { KlPlatThreadFn fn; void *arg; } WinStart;
...
int  kl_plat_mutex_init(KlPlatMutex *m)    { if (!m) return -1; InitializeSRWLock(&m->l); return 0; }
void kl_plat_mutex_lock(KlPlatMutex *m)    { if (m) AcquireSRWLockExclusive(&m->l); }
void kl_plat_mutex_unlock(KlPlatMutex *m)  { if (m) ReleaseSRWLockExclusive(&m->l); }
void kl_plat_mutex_destroy(KlPlatMutex *m) { (void)m; }   /* SRW locks own no resource to release */

int  kl_plat_cond_init(KlPlatCond *c)      { if (!c) return -1; InitializeConditionVariable(&c->c); return 0; }
void kl_plat_cond_signal(KlPlatCond *c)    { if (c) WakeConditionVariable(&c->c); }
void kl_plat_cond_broadcast(KlPlatCond *c) { if (c) WakeAllConditionVariable(&c->c); }
void kl_plat_cond_destroy(KlPlatCond *c)   { (void)c; }   /* likewise */
