...
#include "platform_thread.h"   /* the PAL's threading primitives; no pthreads above this */
#include <string.h>
#include <limits.h>
#include <stdint.h>
...
static void worker_thread(void *arg)
{
    ...
    while (!pool->shutdown) {
        while (pool->work_count == 0 && !pool->shutdown)
            kl_plat_cond_wait(&pool->work_avail, &pool->mutex);
...
KlThreadPool *kl_thread_pool_create(KlEventCtx *ctx, const KlThreadPoolConfig *cfg)
{
    ...
    if ((size_t)num_workers > SIZE_MAX / sizeof(KlPlatThread)) return NULL;
    ...
    if (kl_plat_mutex_init(&pool->mutex) != 0) { ctx->last_error = KL_ERR_THREAD; goto fail_pipe; }
    if (kl_plat_cond_init(&pool->work_avail) != 0) { ctx->last_error = KL_ERR_THREAD; goto fail_mutex; }
    ...
    for (int i = 0; i < num_workers; i++) {
        if (kl_plat_thread_create(&pool->threads[i], worker_thread, pool) != 0)
            break;
        started++;
    }
...
fail_threads:
    kl_free(alloc, pool->threads, (size_t)num_workers * sizeof(KlPlatThread));
fail_watcher:
    kl_watcher_del(ctx, pool->wakeup.rd);
fail_cond:
    kl_plat_cond_destroy(&pool->work_avail);
fail_mutex:
    kl_plat_mutex_destroy(&pool->mutex);
fail_pipe:
...
int kl_thread_pool_submit(KlThreadPool *pool, const KlWorkItem *item)
{
    ...
    kl_plat_cond_signal(&pool->work_avail);
    kl_plat_mutex_unlock(&pool->mutex);
...
void kl_thread_pool_free(KlThreadPool *pool)
{
    ...
    kl_plat_mutex_lock(&pool->mutex);
    pool->shutdown = 1;
    kl_plat_cond_broadcast(&pool->work_avail);
    kl_plat_mutex_unlock(&pool->mutex);

    /* Join all workers */
    for (int i = 0; i < pool->num_workers; i++)
        kl_plat_thread_join(&pool->threads[i]);
