...
static const char *kl_http_cors_default_methods = "GET, POST, OPTIONS";
static const char *kl_http_cors_default_headers = "Content-Type, Authorization";

/* ── Public API ─────────────────────────────────────────────────────── */

void kl_http_cors_init(KlHttpCorsConfig *config) {
    memset(config, 0, sizeof(*config));
    config->allowed_methods = kl_http_cors_default_methods;
    config->allowed_headers = kl_http_cors_default_headers;
...
int kl_http_cors_add_origin(KlHttpCorsConfig *config, const char *origin) {
    ...
    if (config->origin_count >= KL_HTTP_CORS_MAX_ORIGINS) return 0;

    size_t len = strlen(origin);
    if (len >= KL_HTTP_CORS_ORIGIN_SIZE) len = KL_HTTP_CORS_ORIGIN_SIZE - 1;
...
int kl_http_cors_parse_origins(KlHttpCorsConfig *config, const char *origins) {
    ...
    while (*p) {
        ...
        if (end > start) {
            ...
            if (len >= KL_HTTP_CORS_ORIGIN_SIZE) len = KL_HTTP_CORS_ORIGIN_SIZE - 1;
            char buf[KL_HTTP_CORS_ORIGIN_SIZE];
            memcpy(buf, start, len);
            buf[len] = '\0';
            if (kl_http_cors_add_origin(config, buf)) added++;
        }
...
int kl_http_cors_middleware(KlHttpRequest *req, KlHttpResponse *res, void *user_data) {
    ...
    const KlHttpCorsConfig *config = user_data;
    ...
    if (!kl_http_cors_is_allowed(config, origin, origin_len)) return 0;

    /* Determine the Allow-Origin value */
    char origin_buf[KL_HTTP_CORS_ORIGIN_SIZE];
    const char *allow_origin;
    if (config->origin_count == 0) {
        allow_origin = "*";
    } else {
        /* Echo back the specific origin */
        size_t len = origin_len < KL_HTTP_CORS_ORIGIN_SIZE - 1
                     ? origin_len : KL_HTTP_CORS_ORIGIN_SIZE - 1;
