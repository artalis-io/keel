...
int kl_http_router_init(KlHttpRouter *r, KlAllocator *alloc) {
    ...
    r->routes = kl_malloc(alloc, sizeof(KlHttpRoute) * (size_t)r->capacity);
    return r->routes ? 0 : -1;
}
...
int kl_http_router_add(KlHttpRouter *r, const char *method, const char *pattern,
    ...
    if (r->count >= r->capacity) {
        ...
        KlHttpRoute *new_routes = kl_realloc(r->alloc, r->routes,
                                         sizeof(KlHttpRoute) * (size_t)r->capacity,
                                         sizeof(KlHttpRoute) * (size_t)new_cap);
        if (!new_routes) return -1;
        r->routes = new_routes;
        r->capacity = new_cap;
    }

    r->routes[r->count] = (KlHttpRoute){
        .method = method,
        .pattern = pattern,
...
int kl_http_router_add_streaming(KlHttpRouter *r, const char *method, const char *pattern,
    ...
    int rc = kl_http_router_add(r, method, pattern, handler, user_data, body_reader);
    if (rc < 0) return rc;
    /* kl_http_router_add appended to slot count-1. Mark it streaming. */
...
int kl_http_router_use(KlHttpRouter *r, const char *method, const char *pattern,
    ...
    if (r->mw_count >= r->mw_capacity) {
        ...
        size_t old_size = sizeof(KlHttpMiddlewareEntry) * (size_t)r->mw_capacity;
        size_t new_size = sizeof(KlHttpMiddlewareEntry) * (size_t)new_cap;
        KlHttpMiddlewareEntry *new_mw = kl_realloc(r->alloc, r->middleware,
                                                old_size, new_size);
        if (!new_mw) return -1;
        r->middleware = new_mw;
    ...
    r->middleware[r->mw_count] = (KlHttpMiddlewareEntry){
        .method = method,
        .pattern = pattern,
        .method_len = strlen(method),
        .pattern_len = strlen(pattern),
...
int kl_http_router_run_middleware(KlHttpRouter *r, KlHttpRequest *req, KlHttpResponse *res) {
    for (int i = 0; i < r->mw_count; i++) {
        KlHttpMiddlewareEntry *mw = &r->middleware[i];
...
int kl_http_router_use_post(KlHttpRouter *r, const char *method, const char *pattern,
    ...
    if (r->post_mw_count >= r->post_mw_capacity) {
        ...
        size_t old_size = sizeof(KlHttpMiddlewareEntry) * (size_t)r->post_mw_capacity;
        size_t new_size = sizeof(KlHttpMiddlewareEntry) * (size_t)new_cap;
        KlHttpMiddlewareEntry *new_mw = kl_realloc(r->alloc, r->post_middleware,
                                                old_size, new_size);
        if (!new_mw) return -1;
    ...
    r->post_middleware[r->post_mw_count] = (KlHttpMiddlewareEntry){
        .method = method,
        .pattern = pattern,
        .method_len = strlen(method),
        .pattern_len = strlen(pattern),
...
int kl_http_router_dispatch_synthetic(KlHttpRouter *r, KlHttpRequest *req,
    ...
    int num_params = 0;
    int match_status = kl_http_router_match(r, req->method, req->method_len,
                                       req->path, req->path_len,
    ...
    if (run_middleware) {
        int mw_rc = kl_http_router_run_middleware(r, req, res);
        if (mw_rc != 0) return mw_rc;

        mw_rc = kl_http_router_run_post_middleware(r, req, res);
        if (mw_rc != 0) return mw_rc;
    }
    ...
    kl_http_response_status(res, match_status);
    return match_status;
}
...
void kl_http_router_free(KlHttpRouter *r) {
    if (r->routes) {
        kl_free(r->alloc, r->routes, sizeof(KlHttpRoute) * (size_t)r->capacity);
        r->routes = NULL;
    }
    if (r->middleware) {
        kl_free(r->alloc, r->middleware,
                sizeof(KlHttpMiddlewareEntry) * (size_t)r->mw_capacity);
        r->middleware = NULL;
    }
    if (r->post_middleware) {
        kl_free(r->alloc, r->post_middleware,
                sizeof(KlHttpMiddlewareEntry) * (size_t)r->post_mw_capacity);
        r->post_middleware = NULL;
    }
}
