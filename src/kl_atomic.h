 ...
 *
 * static inline, like the load/store helpers below, rather than a function in its own TU. Keeping it
 * out of the archive is deliberate: the freestanding server archive is a default-deny symbol set
 * (freestanding-lib-server-selfcontained), so a new exported symbol there is a gate failure by design,
 * and this is a query about a caller's object rather than a service the library provides.
 */
...
static inline int kl_atomic_int_is_lock_free(const int *flag) {
    if (!flag) return 0;
#if KL_ATOMIC_INT_LOCK_FREE_STATIC
