    ...
    while (i < len) {
        ...
        case KL_CHUNK_SIZE: {
            ...
            if (ch >= '0' && ch <= '9')      digit = ch - '0';
                ...
                if (dec->size_digits == 0) {
                    dec->state = KL_CHUNK_ERROR;
                    return -1;
                }
                dec->state = KL_CHUNK_EXT;
                i++;
                break;
            } else if (ch == '\r') {
                if (dec->size_digits == 0) {
        ...
        case KL_CHUNK_SIZE_CR:
            ...
            if (dec->chunk_remaining == 0) {
                ...
                dec->state = KL_CHUNK_TRAILER;
                dec->trailer_cr = 1;
            } else {
                dec->state = KL_CHUNK_DATA;
            }
            i++;
            break;
        ...
        case KL_CHUNK_DATA: {
            ...
            if (dec->chunk_remaining == 0) {
                dec->state = KL_CHUNK_DATA_CR;
                dec->trailer_cr = 0;  /* reset: haven't seen CR yet */
            }
            break;
        }

        case KL_CHUNK_DATA_CR:
        ...
        case KL_CHUNK_TRAILER:
            /* trailer_cr: 1 = at start of line, 0 = mid-line */
            if (data[i] == '\r') {
                dec->state = KL_CHUNK_TRAILER_CR;
            } else {
