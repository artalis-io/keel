/*
 * completion.h — INTERNAL. The platform-independent completion event axis.
 *
 ...
 * consumer (completion_core.c). A backend TU (event_iocp.c) implements it.
 *
 * The generic driver (completion_core.c) consumes KlCompletionEvent's and drives
 ...
 * docs/archive/phases/phase8b_iocp_breadth_design.md.
 */
 ...
 * src/protocols/http/completion_http.h; see docs/archive/freezes/protocols_restructure_freeze.md §4.8. */
...
typedef enum {
    ...
    KL_COMP_CONNECT,  /* an outbound connect finished. The completion mirror of
                       ...
                       * ConnectEx, lwip-raw a tcp_connect+connected_cb. The consumer is
...
typedef struct KlCompletionEvent {
    ...
    int            tos;                      /* UDP_RECV: received TOS/Traffic-Class byte, or -1 = none.
                                              * A backend that captures it (RX_TOS requested)
...
/* ── Completion sub-vtable (runtime-injectable completion axis) ──────
 *
 ...
 * perfectly-predicted branch with no behavioral change.
 *
...
typedef struct KlCompletionOps {
     ...
     * SETUP call that RETURNS the backend's accept window and does NOT post:
     ...
     *   <0   fatal setup error.
     * post_accept posts exactly ONE accept op (post-driven backends). See
     * docs/archive/audits/generic_transport_audit.md + docs/archive/freezes/protocols_restructure_freeze.md §4.8. */
    ...
    /* post_sendfile is neutral over KlStream but HTTP/file-transfer-specific in POLICY — NOT part
     * of the generic stream seam (file transfer is an Optional capability; the generic
     * KlStream promises byte reads/writes only). See docs/archive/audits/generic_transport_audit.md §8. */
     ...
     * (the tagged KlWatcher the client registered on `fd`) — the client-side
     * outbound counterpart of post_accept. See completion.h KL_COMP_CONNECT + the async HTTP client (http_client_async.c). */
    int  (*post_connect)(struct KlEventCtx *ctx, KlSocketHandle fd,
                         const KlSockAddr *addr, void *watcher_udata);
    /* Teardown accept-side FORCE-COMPLETION. Called once at server teardown, AFTER
 ...
 * the first call). Neutral: the HTTP wrapper kl_comp_prime_accepts(KlHttpServer*) in
...
/* Teardown accept-side force-completion: guarantee every posted accept will
 ...
 * KL_COMP_WRITE on completion. Neutral: the HTTP wrapper kl_comp_post_sendfile(KlHttpConn*)
 * forwards &c->stream. */
int kl_comp_post_sendfile_raw(KlStream *stream, const KlIoVec *head_iov, int head_n,
                              size_t head_total, int file_fd, uint64_t count);

/* Post one outbound connect. Declared in completion_io.h (the shared event-model seam)
