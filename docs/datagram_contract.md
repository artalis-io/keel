...
| Packet-slot bounded **send** queue | ✅ (7B-6: KlDatagram fixed-slot over the readiness facade) | ⚙ | ✅ (7B-4: KlDatagram fixed-slot over the live facade) | ✅ (7B-5: live io_uring) | ✅* (7B-7 PROVISIONAL — facade lifecycle done + IOCP CI smoke wired; firm ✅ once the Windows IOCP CI job is green) | ⚙ (no preallocated atomic send-slot queue; the 16-slot ring is receive-side) | ⚙ |
| Strict pause (post no more recv) | ✅ (7B-6: drop-interest latch over the readiness facade) | ⚙ | ✅ (7B-4: strict latch over the live facade) | ✅ (7B-5: live io_uring) | ✅* (7B-7 PROVISIONAL — facade lifecycle done + IOCP CI smoke wired; firm ✅ once the Windows IOCP CI job is green) | ⚙ | ⚙ |
