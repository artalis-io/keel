...
| Packet-slot bounded **send** queue | ⚙ (recast from byte-FIFO) | ⚙ | ✅ (7B-4: KlDatagram fixed-slot over the live facade) | ✅ (7B-5: live io_uring) | ⚙ | ⚙ (no preallocated atomic send-slot queue; the 16-slot ring is receive-side) | ⚙ |
| Strict pause (post no more recv) | ⚙ (drop interest) | ⚙ | ✅ (7B-4: strict latch over the live facade) | ✅ (7B-5: live io_uring) | ⚙ (latch) | ⚙ | ⚙ |
