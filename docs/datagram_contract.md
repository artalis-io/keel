...
| Packet-slot bounded **send** queue | ✅ (7B-6: KlDatagram fixed-slot over the readiness facade) | ⚙ | ✅ (7B-4: KlDatagram fixed-slot over the live facade) | ✅ (7B-5: live io_uring) | ✅* (7B-7 PROVISIONAL — facade lifecycle done + IOCP CI smoke wired; firm ✅ once the Windows IOCP CI job is green) | ✅ (7B-8: live lwIP-raw, container ASan/UBSan/LSan) | ✅* (7B-9 PROVISIONAL — host-mock green: public KlDatagram DETACHED/QUARANTINED close + no-handler router site, ASan/UBSan; firm ✅ once the QEMU/OVMF KlDatagram e2e passes) |
| Strict pause (post no more recv) | ✅ (7B-6: drop-interest latch over the readiness facade) | ⚙ | ✅ (7B-4: strict latch over the live facade) | ✅ (7B-5: live io_uring) | ✅* (7B-7 PROVISIONAL — facade lifecycle done + IOCP CI smoke wired; firm ✅ once the Windows IOCP CI job is green) | ✅ (7B-8: live lwIP-raw, container ASan/UBSan/LSan) | ✅* (7B-9 PROVISIONAL — host-mock green: public KlDatagram close over EFI_UDP4, ASan/UBSan; firm ✅ once the QEMU/OVMF KlDatagram e2e passes) |
