...
- **Batching + QoS** *(throughput)* — **`recvmmsg`/`sendmmsg` done (2026-07-19)** via the `mmsg_batch` knob (transparent, always-on where available, per-datagram fallback elsewhere; see `docs/udp_batching_design.md`). Remaining: UDP GSO/GRO segmentation offload (Phase B) and ECN/TOS/DSCP marking. Matters for high-PPS services and QUIC.
