...
- **DNS extras** *(partly done)* — ~~TCP fallback on the truncation (`TC`) bit~~ **(done, 2026-07-19)**: a persistent, pipelined, idle-closed per-nameserver DNS-over-TCP connection (RFC 7766), routed through a `(fd, KlTls*)` I/O helper so DoT slots in as a wrapping increment. See `docs/dns_tcp_cookies_design.md`. Still deferred: DNS cookies (RFC 7873, a stronger off-path anti-spoof than 0x20 — Phase 2 of the same design), DNSSEC validation, and DoT/DoH transports (DoH rides the existing HTTPS client).
