...
- **Batching + QoS** *(throughput)* — **`recvmmsg`/`sendmmsg` + UDP GSO/GRO done (2026-07-19)**: the `mmsg_batch` knob (transparent batching), `kl_udp_send_gso` (segmentation offload with per-segment fallback), and `recv_gro` (receive coalescing with transparent per-segment split or an opt-in `kl_udp_recv_segments` coalesced callback). All Linux-only with clean fallbacks; see `docs/udp_batching_design.md`. Remaining: ECN/TOS/DSCP marking. Matters for high-PPS services and QUIC.
