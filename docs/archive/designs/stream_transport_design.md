# Keel Stream Transport Contract (Tier-1 shipped; Tier-2/Tier-3 not yet normative)

> **ARCHIVED: historical design record, not the current contract.** This is the original
> stream-transport design/planning document (Phase A/B/C sequencing and proposed extensions),
> retained for history. It is **not normative** and does not track the shipped API. The current,
> living contract is [contracts/stream.md](../../contracts/stream.md).

**Date:** 2026-08-08
**Status: Tier-1/Phase-B surface SHIPPED; not yet fully normative.** A prior version was labelled
"normative" while promising capabilities Keel did not have; that was demoted to DRAFT. The
Tier-1/Phase-B baseline surface is now **implemented and shipped**; `KlStream`
(`include/keel/stream.h`), `KlListener` (`include/keel/listener.h`), and `KlConnectOp`
(`include/keel/connect_op.h`) all exist with the refined names recorded below. What remains unbuilt
is Tier 2 (the precise close taxonomy via a `KlIoStatus status` field on `KlCompletionEvent`,
per-op operation identity, the TLS-wrapped-stream state machine) and Tier 3 (half-close/abort,
tagged `KlEndpoint`, sendfile extensions). This revision (a) records that the Tier-1 surface exists,
(b) tiers the semantics into what exists vs. must-be-built vs. optional, and (c) records the
resolved design decisions for the must-be-built parts. It becomes **fully** normative only after the
Tier-2 machinery lands (see `docs/archive/audits/generic_transport_audit.md` §8).
...
> **Phase A/B/C/Optional** plan in `docs/archive/audits/generic_transport_audit.md` §8. The mapping is exact:
...
- **Tier 1 = Phase B (baseline public surface):** push read-delivery callback; `kl_stream_write`
  with an **atomic-accept** result (over a **preallocated** bounded queue; no hot-path alloc;
  oversized → `TOO_LARGE`); low-water writable notification; stream-level strict
  `pause`/`resume`; **stream-level cancellation**; graceful `kl_stream_close` + detachment `on_close`;
  a cancellable `KlConnectOp` (client) and a **push `on_accept`** listener; **plaintext only**. (A
  scatter-gather `kl_stream_writev` is **proposed, not yet built**; only `kl_stream_write` ships;
  the §3 discussion is a design sketch.) On read termination Tier 1 delivers a **conservative
  close**: the read callback `kl_stream_on_recv(s, len, ok)` reports `ok == 0` ("the stream ended;
  cause unknown"), **not** a claim of orderly EOF; today's `KlCompletionEvent` carries only
  `bytes`+`ok` and collapses orderly EOF and error into `ok == 0`, so orderly-EOF-vs-error is **not**
  representable on completion backends yet. There is **no `recv≤0 → EOF` rule**; that is not a valid
  generic-stream rule. *(The write-result type, the bounded-queue backpressure API, and
  deferred-destruction/detachment ownership are all part of THIS tier; they are new machinery, but
  they are Tier-1/Phase-B work, not Tier 2. Tier 2 does not re-introduce them.)*
  ...
  **TLS-wrapped-stream** state machine. Tier 2 refines Tier-1 capabilities; it does not restate them.
  - **The close taxonomy (why it is Phase C):** `KlCompletionEvent` carries just `bytes`+`ok` today,
    ...
    **`KlIoStatus status`** field to `KlCompletionEvent`; the driver then classifies:
    ...
    Until that field lands, Tier 1's read termination is only the conservative `ok == 0` on
    `kl_stream_on_recv` (cause unknown). *(Today `KlCompletionEvent` carries only `bytes`+`ok`; the
    `KlIoStatus status` field is genuinely future Tier-2 work, not present in `src/completion.h`.)*
- **Tier 3 (OPTIONAL: NOT in the initial contract):** `shutdown_write` half-close, `abort`/RST
  ...
  `KlEndpoint` union (**proposed only; no `KlEndpoint` symbol exists today; the shipped API uses
  `KlSockAddr` for connect/bind addresses**, per decision #6), file-transfer (`sendfile`) extensions.

### Resolved decisions (this DRAFT commits to these; sections below are read through them)

1. **Write acceptance = ATOMIC.** `kl_stream_write` accepts *all* bytes or *none*; it returns the
   bare enum `KlStreamWriteStatus` (`include/keel/stream.h`), for which `accepted ∈ {0, len}` is
   **implicit in the status** (`KL_STREAM_ACCEPTED` ⇒ all `len`; every other status ⇒ 0). Maps
   naturally onto bounded buffering + today's `KlDrain` (0/-1). *(A richer result struct carrying
   explicit `accepted`/`io_status` fields is **proposed, not built**; the shipped return is the bare
   enum.)* Prefix-acceptance is a possible later change (requires extending `KlDrain`); not the
   initial policy.
...
- `KlStream`; an ordered, connected, reliable byte stream (TCP / Unix / lwIP TCP; TLS-wrapped and
  future pipe/vsock are later tiers). Not a datagram (see the sibling `KlDatagram` primitive and its
  contract `docs/contracts/datagram.md`). It carries its own
...
- Addresses; the shipped API uses **`KlSockAddr` directly** (already neutral IPv4/IPv6/Unix). It is
  address identity only and carries **no provider/transport kind**; provider selection stays in
  `KlEventCtx`/config (§9). *(A named `KlEndpoint` type is **proposed only** for a later tier; no such
  symbol exists today; decision #6 resolves endpoints to `KlSockAddr`.)* A tagged non-socket union
  is a later tier.
...
- **Short/fragmented reads:** a read callback delivers *whatever arrived*; 1 byte or many. The
  ...
  count."
- **Read termination:**
  - **Tier 1 (conservative):** when the read side ends, the read callback `kl_stream_on_recv(s, len,
    ok)` reports `ok == 0`: "the stream ended; cause not distinguishable." There is **no
...
- **Atomic acceptance.** `kl_stream_write` (and the proposed `kl_stream_writev`) accept **all** the
  bytes or **none**; there is no caller-visible partial write. The shipped return is the **bare enum
  `KlStreamWriteStatus`** (`include/keel/stream.h`); acceptance (0 vs `len`) is implicit in the
  status, and a category code for the error case travels with the stream's `last_error`. *(A richer
  result struct; `KlStreamWriteResult { KlStreamWriteStatus status; size_t accepted; KlIoStatus
  io_status; }`; is **proposed, not built**; the shipped API returns the bare enum below.)*
  | `KlStreamWriteStatus` | accepted | meaning |
  |---|---|---|
  ...
  | `KL_STREAM_ERROR` | `0` | fatal; the category is recorded on the stream (a proposed result struct would carry it as `io_status`) |
  ...
  acceptance.
- **[PROPOSED: `kl_stream_writev` is not yet built; only `kl_stream_write` ships.]** When a
  scatter-gather `kl_stream_writev` is added, its length computation + input validation are NORMATIVE
  and must precede the capacity check (Finding 5). The total-length sum feeds the atomic reservation,
  so it must be computed safely or the all-or-none check itself could overflow. Before reserving or
  comparing to `total_capacity`, `kl_stream_writev` must, in order:
  - reject `iovcnt < 0` or `iovcnt` beyond the supported/provider limit (e.g. `IOV_MAX`,
    `MSG_MAXIOVLEN`, or the backend's `WSABUF`/`iovec` cap) → `KL_STREAM_ERROR` / `KL_IO_FATAL`;
...
**[PROPOSED: not yet built.]** Only `kl_stream_write` ships today; `kl_stream_writev` does not
exist in `include/keel/stream.h`. The design below records the intended semantics for when it is
added; the symbol is not available yet.
...
write returns a bare `KlStreamWriteStatus` (§2), one of (atomic; the accepted count is `len` or `0`,
implicit in the status):
...
| `KL_STREAM_ERROR` | `0` | fatal; the category is recorded on the stream (a proposed result struct would carry it as `io_status`); begin teardown |
...
|---|---|---|
| peer sent FIN (orderly) | 1→2 | **Tier 1:** `kl_stream_on_recv` `ok == 0` (cause not distinguishable); **Tier 2:** orderly **EOF** (needs `KlCompletionEvent.status`, §header). (Tier-3 half-open keeps the write side open.) |
| peer sent RST | 1→2 | **Tier 1:** `kl_stream_on_recv` `ok == 0` (NOT "EOF-like"; an unknown termination is not a clean message boundary); **Tier 2:** read **error** `KL_IO_RESET` (needs `KlCompletionEvent.status`, §header) |
...
- **Handshake-before-delivery, and its failure ownership (Finding 2):** a newly connected/accepted TLS
  ...
  - **Accepted (server/listener):** the listener/TLS wrapper still owns the **half-built** stream. On
    ...
    application-ready" promise). The shipped listener delivers this accept-failure notification to
    the backend via **`kl_listener_on_accept_failed(listener, error)`** (`include/keel/listener.h`),
    which returns the reserved credit and never conveys stream ownership; success is delivered by the
    backend calling **`kl_listener_on_accepted`**, which invokes the `on_accept` hook. `on_accept`
    fires **only** on handshake success. This preserves the invariant that every stream
...
- **The DETACHMENT rule covers EVERY external reference into stream storage; reads AND writes, not
  ...
  - **(1) Backend-owned copy/scratch.** The op references stable storage the *backend* owns; never the
    caller's `KlStream`. Reads: TLS recv reuses the **plaintext READ op** (`IOU_READ`/`KL_IOCP_READ`/
    `PC_READ`; there is no dedicated TLS-recv op), but for a TLS conn the target buffer is the HTTP
    adapter's **per-conn ciphertext scratch** (not caller `KlStream` storage), which the adapter
    (`comp_on_read`) then feeds to the engine via `feed_input`; plus EFI plaintext (the firmware
    `ReceiveData` runs *synchronously inside drain*, so no firmware op holds `read_buf`
...
- **Connect:** the shipped client connect is `kl_connect_op_init(KlConnectOp *op, const
  KlConnectOpHooks *hooks, void *ctx)` followed by `kl_connect_op_start(op)`
  (`include/keel/connect_op.h`), driven by the `KlConnectOpHooks` callbacks. It is an async op with
  the §6.3 terminal set: **connected** | **refused** (`KL_IO`/`KL_ERR_CONNECT`) | **timed-out** |
  **cancelled**. Cancellation racing a successful connect resolves per §6.3 (completion wins if it
  already happened). The client's Happy-Eyeballs address racing is an implementation of this op,
...
- **Retyped ops:** `prime_accepts(KlListener*)` / `post_accept(KlListener*)`; the completion event
  identifies its target as `KL_COMP_ACCEPT.target = KlListener*`. *(Phase A used a transitional
  server-owned `KlAcceptTarget`; a `{ KlHttpServer *server; }` accept-routing adapter embedded in
  `KlHttpServer`; **Phase B completed the rename: `KlListener` is the real type now and
  `KlAcceptTarget` has been removed**; see the "KlAcceptTarget removed" note in
  `src/protocols/http/completion_http_server.c`. The ACCEPT completion now recovers the server from
  the event ctx.)*
...
- **If reservation itself fails**: on the **conforming (preallocated, allocation-free) path** the only
  ...
  **closes that handle, restores the credit via `kl_listener_on_accept_failed`, and re-arms only if
  the listener is still live.** A generic acquire hook is thus allowed to fail; the contract just
  never lets that failure surface *after*
  `on_accept` has handed a stream over. "Credit" always means reserved backing, never a hope.
- **`on_accept` reentrancy (and the `kl_listener_on_accept_failed` path); liveness lives in a
  backend-owned token, never a re-read of the possibly-freed listener (Finding 2).** The callback
  runs on the loop thread and **may
  ...
  - Equivalently, a backend MAY do **all** re-arm/credit bookkeeping **before** invoking the callback
    and treat `on_accept` (and the `kl_listener_on_accept_failed` notification) as an **unconditional
    destructive tail** (no post-callback dispatch work at all). Either is conforming.
...
- **`on_accept` conveys ownership; a stream that fails before `on_accept` is never handed over
  ...
  credit-lease release) and reports the failure via **`kl_listener_on_accept_failed`**; but **no
...
- `kl_stream_write` returns the bare enum `KlStreamWriteStatus` with **atomic** acceptance (accepted
  is `len` or `0`, implicit in the status). A write `> total_capacity` returns `TOO_LARGE`
  (permanent); the caller **chunks** logical messages larger than the bounded queue (§2). (The
  proposed `kl_stream_writev`, not yet built, must match `write` when added.)
...
- **Read termination** (Tier 1) surfaces as `kl_stream_on_recv` with `ok == 0`: "the stream ended;
  cause not distinguishable." There is **no `recv≤0 → EOF` rule**; distinguishing orderly FIN from
  reset/error is a Phase-C capability (the `KlIoStatus status` field, §header).
...
- **Callback frees the listener (Finding 2):** an `on_accept` that synchronously frees the listener
  (or stops accepting), and likewise the `kl_listener_on_accept_failed` path, does not cause a
  post-callback read of the freed listener; the driver consults the backend-owned token, does not
  re-arm, and is ASan-clean.
...
- **Accepted-TLS handshake failure ownership (Finding 2):** a failed/timed-out accepted handshake fires
  **no `on_accept` and no stream `on_close`**: only internal teardown + credit-lease return (reported
  via `kl_listener_on_accept_failed`); the credit is reclaimed so the listener can arm the next accept. Assert
