...
Status: COMPLETE (archived). This was the FROZEN-for-review plan (F2-0, revised); every increment it
sequences has since shipped and its gates run in standing CI. It is retained as the design-of-record
for the F2 public-surface hardening phase. The live, machine-consumed inventories and classification
manifests remain under `docs/f2/` (regenerated by `tools/f2_public_inventory.sh`); this document is
the frozen narrative, not a live manifest.

Completion map (planned increment -> shipped artifact):

- F2-1a inventories + scaffolding -> `docs/f2/{public_functions.tsv,public_types.tsv,public_headers.txt,type_classification.tsv,function_coverage.tsv,allocator_boundaries.tsv}`, generated + checked by `tools/f2_public_inventory.sh --check --selftest`.
- F2-A C++ linkage ruling -> `docs/archive/f2/f2_c_extensibility_decision.md` policy; `extern "C"` obligation enforced by `tools/f2_cxx_link_test.sh`.
- F2-C allocator-boundary classification -> `docs/f2/allocator_boundaries.tsv` + `make check-allocator-boundaries`.
- F2-D event-loop-fd ruling -> `docs/archive/f2/f2_d_eventloop_fd_decision.md` + `make check-no-eventloop-fd`.
- F2-B public-layout decision -> `docs/archive/f2/f2_b_public_layout_decision.md`; opacity enforced by `tools/f2_opaque_probe.sh`.
- F2-1b header contract gate -> `make check-public-headers` (standalone C11/C++11 compile + inventory/classification joins + linkage/opacity canaries).
- F2-2 install/uninstall hardening -> manifest-only install, config-triggered `keel.pc` regen, ownership-safe uninstall; `make check-install`.
- F2-3 installed-consumer gate -> `make check-installed-consumer` (out-of-tree C consumer via pkg-config; PREFIX + DESTDIR; leakage + flags checks).
- F2-4 public-function coverage -> default-deny `docs/f2/function_coverage.tsv` + `make check-public-coverage`.
- F2-5 coverage closure -> P1/P2 (+ cheap P3) gaps closed with direct behavioral tests (see `docs/f2/coverage_gaps.md`).
- F2-6 reconciliation -> the six gates above enrolled in the standing CI gates job; this archive.

Historical grounding: FROZEN for review (F2-0, revised). Docs-only at freeze time. Grounded in the
live tree at base commit `4e667e1` on branch `main`, not in the roadmap's assumptions; the mechanical
claims (standalone compile, staged install, DESTDIR staging, out-of-tree consumer) were reproduced
locally.
