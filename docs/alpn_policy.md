...
TCP → TLS handshake → ALPN result
        h2        → HTTP/2 adapter (h2.c + a KlH2ServerSession, e.g. nghttp2)
        http/1.1  → HTTP/1.1 adapter (connection.c + the llhttp parser)
        (none)    → HTTP/1.1 adapter
   → one KlRequest / KlResponse
   → one KlRouter (kl_router_match)
   → one middleware chain (kl_router_run_middleware)
   → one KlHandler
```
...
(`kl_conn_on_handshake`, `src/connection.c`). The selected protocol is immutable
...
- After the handshake, `kl_conn_on_handshake` reads the negotiated protocol
  (`KlTls::alpn_protocol`):
  - `h2` **and** the server has an HTTP/2 config (`KlConfig.h2`) → the HTTP/2
    adapter owns the connection (`KL_CONN_HTTP2`).
...
- If the server does **not** advertise `h2` (or has no `KlConfig.h2`), it serves
  HTTP/1.1 only; a client that somehow selected `h2` would not be misrouted
  because the h2 path is gated on `KlConfig.h2`.
...
- The HTTP/2 client (`kl_h2_client_connect`) verifies the negotiated protocol
  after the handshake:
...
the same `KlRequest`/`KlResponse`, match on the same `KlRouter`, run the same
middleware, and call the same `KlHandler`. Handlers see no ALPN result, HTTP/2
frames, stream IDs, HPACK state, or HTTP/1.1 parser internals — only
`KlRequest`/`KlResponse`.

**Authority convergence:** HTTP/2's `:authority` pseudo-header is injected as a
synthetic `host` header, so `kl_request_header(req, "host")` returns the
...
- **`tests/test_alpn.c`** (core `make test`, deterministic, no crypto): drives
  `kl_conn_on_handshake` with a mock TLS whose negotiated ALPN is configurable and
