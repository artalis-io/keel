...
**covered by a host mock-EFI harness** (`mock_efi_test.c`, 16 scenarios) that compiles the *real*
...
**Method.** Host mock-EFI harness (16 scenarios, ASan+UBSan+LSan (`detect_leaks=1` on Linux; `=0` on Darwin)) — the deliberate
...
| U10 | **Medium** | `mbedtls_config_uefi.h`; new `civil_time`, `wallclock_uefi`, `clock_snapshot`, `time_uefi`; `platform_uefi.c`; `spikes/uefi/efi_min.h` | Certificate **validity-time was not enforced** (`MBEDTLS_HAVE_TIME` off) — TLS accepted expired / not-yet-valid certs (CA + hostname only). | Enable `HAVE_TIME`/`HAVE_TIME_DATE`; bind mbedTLS's clock to Runtime Services **GetTime** → pure `civil_time` math. Trust boundary (3rd/4th review): **per-session SNAPSHOT** (`clock_snapshot.c`) captured once at session creation + advanced by the monotonic clock — `mbedtls_time` never calls GetTime mid-handshake (closes the "GetTime fails after setup → epoch 0 → 1970-cert" loophole); **structural fail-closed gate** refuses the TLS session with no trustworthy clock; **`EFI_TIME` field validation** rejects malformed firmware pre-conversion; **unspecified-TZ rejected by default** (UEFI §8.3 local-time; app offset or `KL_UEFI_ASSUME_UNSPECIFIED_UTC` test flag to opt in), sign `UTC = local + TimeZone`. | `t_civil_time` + `t_wallclock_decode` (unspecified-TZ policy, malformed fields) + `t_clock_snapshot` (snapshot read-not-GetTime; **post-snapshot GetTime failure does not reopen epoch-0**; failed refresh → epoch 0). **QEMU U-4 prod: valid+good-clock → 200; valid+bad-clock (RTC 2000) → TLS refused, no 200; expired (2020) → `status -1`/`KL_ERR_TLS_HANDSHAKE`/no 200.** |
