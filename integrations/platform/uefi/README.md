...
This is **U-1**, the first stage of the real EFI provider (Phase 10, F-8). It
...
container run --rm --cpus 6 -m 4g -v "$PWD:/src" docker.io/library/ubuntu:24.04 bash -c '
  ...
  cp -a /src/. /work/ && cd /work/integrations/platform/uefi
  BOOT_TIMEOUT=180 KEEL_ROOT=/work bash tests/run.sh'
```
