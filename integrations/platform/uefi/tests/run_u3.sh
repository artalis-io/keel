#!/usr/bin/env bash
# run_u2.sh — U-3 harness. Runs inside the Ubuntu 24.04 container.
#   1. build the freestanding self-contained archive (make freestanding-lib-selfcontained)
#   2. build BOOTX64.EFI (build_u3.sh)
...
#   6. capture serial, grep for U-3: markers / the status line
#
# Env:
...
# -fno-builtin, tripping the self-contained gate — a toolchain quirk, not a U-3
...
# ---- 2. build the EFI app ----
TARGET_PORT="$TARGET_PORT" ./build_u3.sh || { echo "BUILD FAILED"; exit 2; }
...
timeout "${BOOT_TIMEOUT}" qemu-system-x86_64 "${QEMU_ARGS[@]}" 2>&1 | tee /tmp/serial_u3.log
echo "=== serial output end ==="

echo "=== grep markers ==="
grep -E 'U-3:|HTTP/1\.[01]' /tmp/serial_u3.log || echo "(no U-3 markers found)"
