 ...
 *     KlHttpServerConfig.h2 = { .factory = kl_http2_nghttp2_server_session }, entered via h2c
 *     prior-knowledge (the "PRI * HTTP/2.0" preface), routing GET /hello; and
 *   - the CLIENT adapter through kl_http2_client_connect() over a cleartext http://
...
int main(void) {
    ...
    /* Client: real kl_http2_client_connect over cleartext h2c, nghttp2-backed. */
    KlEventCtx ev;
    if (kl_event_ctx_init(&ev, &alloc) < 0) return fail("event ctx init");
    KlHttp2ClientConfig ccfg = { .session = kl_http2_nghttp2_client_session };

    CliState st = {0};
    KlHttp2ClientConn *c = kl_http2_client_connect(&ev, &alloc, &ccfg, url, on_error, &st);
    if (!c) { kl_event_ctx_free(&ev); return fail("client connect"); }

    /* kl_http2_client_request returns -1 until the async connect reaches ACTIVE
    ...
    for (int i = 0; i < 500 && !st.done; i++) {
        if (sid < 0 && !st.err)
            sid = kl_http2_client_request(c, "GET", "/hello", NULL, 0, NULL, 0, on_response, &st);
        if (kl_event_ctx_run(&ev, 16, 20) < 0) break;
    }
    if (sid < 0 && !st.err) { kl_http2_client_free(c); kl_event_ctx_free(&ev);
