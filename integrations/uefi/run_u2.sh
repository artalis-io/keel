...
#   6. capture serial, grep for U-2: markers / the status line
#
# Env:
...
# ---- 2. build the EFI app ----
TARGET_PORT="$TARGET_PORT" ./build_u2.sh || { echo "BUILD FAILED"; exit 2; }
...
mkdir -p wwwroot
printf 'U-2 socket-provider responder OK\n' > wwwroot/index.html
( cd wwwroot && python3 -m http.server "$TARGET_PORT" --bind 0.0.0.0 ) >/tmp/httpd_u2.log 2>&1 &
HTTPD_PID=$!
sleep 1
...
timeout "${BOOT_TIMEOUT}" qemu-system-x86_64 "${QEMU_ARGS[@]}" 2>&1 | tee /tmp/serial_u2.log
echo "=== serial output end ==="

echo "=== grep markers ==="
grep -E 'U-2:|HTTP/1\.[01]' /tmp/serial_u2.log || echo "(no U-2 markers found)"
