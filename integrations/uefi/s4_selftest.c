 ...
 *
 * The server mirror of U-3: a STOCK freestanding KlHttpServer serves an HTTP/1.1
 ...
 *   kl_http_server_route(GET "/", handler)       → 200 + a short body (borrowed static)
...
int efi_main(EFI_HANDLE image_handle, EFI_SYSTEM_TABLE *st) {
     ...
     * OVMF has no TCP4 stack that returns NULL and kl_http_server_init fails below. */
    ...
    /* cfg.sockets == NULL → kl_http_server_init adopts kl_event_native_provider (the EFI
     * socket provider) for the completion loop. Same masking the client relies on. */

    KlHttpServer s;
    if (kl_http_server_init(&s, &cfg) != 0) {
        print("S-4: kl_http_server_init failed (err="); print_int((int)s.last_error); print_line(")");
