...
CORE_SRC = src/allocator.c src/allocator_default_stdlib.c src/kl_cstr.c src/error.c src/version.c src/sockaddr.c $(SOCKET_SRC) $(UNIX_NODE_SRC) $(PLATFORM_SRC) $(PLATFORM_WAKEUP_SRC) $(PLATFORM_SOCKET_SRC) src/protocols/http/http_response.c src/protocols/http/http_router.c \
           ...
           src/protocols/http2/http2_server.c src/protocols/http2/http2_client.c src/thread_pool.c $(PLATFORM_THREAD_SRC) src/wakeup.c src/url.c \
...
# IOURING_TEST_SUITES (vetted against a completion backend), minus the exclusions below; every
# member was confirmed to pass twice in a row over IOCP before being enrolled.
#
# stream_single_shot is a BARE-stream test (raw kl_comp_post_recv/_send + drain, no HTTP connection
# driver), so it validates the IOCP single-shot completion contract natively; it skips on a
...
check-release-artifacts:
	@MAKE="$(MAKE)" sh tools/release_verify.sh --selftest
	@MAKE="$(MAKE)" sh tools/release_verify.sh --verify

check-release-artifacts-strict:
	@MAKE="$(MAKE)" sh tools/release_verify.sh --selftest
	@MAKE="$(MAKE)" sh tools/release_verify.sh --strict

# Local workflow lint (NOT a CI gate): parse every workflow as YAML and run actionlint for workflow/action validity
	...
	if [ -n "$$py" ]; then \
	  for f in .github/workflows/*.yml; do \
	    $$py -c "import sys,yaml; yaml.safe_load(open(sys.argv[1]))" "$$f" || \
	      { echo "check-workflows: FAIL - $$f is not valid YAML"; exit 1; }; \
	  done; \
	else \
	  echo "check-workflows: SKIPPED the YAML parse (no python with PyYAML; tried python3, python)"; \
	fi; \
...
check-install:
	@MAKE="$(MAKE)" sh tools/f2_install_test.sh
...
check-installed-consumer:
	@MAKE="$(MAKE)" sh tools/f2_installed_consumer.sh
...
check-public-headers:
	...
	@MAKE="$(MAKE)" sh tools/f2_standalone_headers.sh
	@MAKE="$(MAKE)" sh tools/f2_opaque_probe.sh >/dev/null && echo "opaque-probe: OK (opaque public types reject layout access; pointer use links; C + C++)"
	@MAKE="$(MAKE)" sh tools/f2_cxx_link_test.sh >/dev/null && echo "cxx-link: OK (C++ links the C archive with guards; unguarded linkage fails)"
...
check-no-httplegacy:
	@sh tools/check_no_httplegacy.sh
...
# Em-dash gate (C3-7). Rejects the exact codepoint U+2014 anywhere in a tracked, non-binary file
# (git ls-files -z, only vendor/ excluded, binaries skipped by a raw-byte NUL check). Full-text, no format allowlist
# and no other exceptions. The script is itself em-dash-free (the four
# glyphs it reasons about are written as \x byte escapes). Positive + negative self-canaries
