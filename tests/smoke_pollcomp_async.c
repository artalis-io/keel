...
int main(void) {
    ...
    for (int i = 0; i < 50 && !ok; i++) {
        nap_ms(50);
        ok = fetch_ok(&alloc, &ccfg, "http://127.0.0.1:18099/async");
    }
    int ok_nostate = ok && fetch_ok(&alloc, &ccfg, "http://127.0.0.1:18099/nostate");
    ...
    if (!ok || !ok_nostate) {
        fprintf(stderr, "smoke-pollcomp-async: async/thread-pool roundtrip FAILED (explicit-resume=%d, response-only-resume=%d)\n",
                ok, ok_nostate);
        return 1;
    }
