...
static void inbound_fill(KlDgramSlots *slots, const void *payload, size_t copy_len,
...
static int comp_arm(void *ctx) {
    ...
    if (c->inline_remaining > 0) {
        ...
        if (c->no_peer) memset(&kl_dgram_slots_inbound(c->slots)->peer, 0,
                               sizeof(kl_dgram_slots_inbound(c->slots)->peer));
...
static void start_comp16(KlDgramRecv *r, KlDgramSlots *slots, KlAllocator *a, CompCtx *c) {
    kl_dgram_slots_init(slots, a, 2, 16, 16);   /* fixed valid args → 0 (asserted via later behavior) */
...
static void fin_recv(KlDgramRecv *r, KlDgramSlots *slots) {   /* retire the trailing re-armed op + free */
...
UTEST(dgram_recv, exact_capacity_no_truncation) {
    KlAllocator a = kl_allocator_default(); KlDgramSlots slots; KlDgramRecv r; CompCtx c;
...
UTEST(dgram_recv, peer_mandatory_missing_fails_safe) {
    ...
    memset(&kl_dgram_slots_inbound(&slots)->peer, 0,          /* strip the peer (provider bug) */
           sizeof(kl_dgram_slots_inbound(&slots)->peer));
...
UTEST(dgram_recv, local_absent_and_stale_cleared) {
    ...
    kl_sockaddr_from_ipv4(&kl_dgram_slots_inbound(&slots)->local,
