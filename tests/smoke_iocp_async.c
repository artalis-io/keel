...
static void on_resume(KlAsyncOp *op, void *ud) { (void)ud; op->conn->state = KL_HTTP_CONN_SENDING; }
...
int main(void) {
    ...
    for (int i = 0; i < 50 && !ok; i++) {
        Sleep(50);
        ok = fetch_ok(&alloc, &ccfg, "http://127.0.0.1:18100/async");
    }
    int ok_nostate = ok && fetch_ok(&alloc, &ccfg, "http://127.0.0.1:18100/nostate");
    ...
    if (!ok || !ok_nostate) {
        fprintf(stderr, "smoke-iocp-async: async/thread-pool roundtrip FAILED (explicit-resume=%d, response-only-resume=%d)\n",
                ok, ok_nostate);
        return 1;
    }
