...
UTEST(dgram_life, owner_only) {
    ...
    g_final = 0;
    KlDgramLife *l = kl_dgram_life_create(&a, (void *)0x1234, on_final, NULL, KL_DGRAM_OWNER_DATAGRAM, (KlDgramDispatchFn)0);
    ASSERT_TRUE(l != NULL);
...
UTEST(dgram_life, mark_dead_then_release) {
    KlAllocator a = kl_allocator_default();
    g_final = 0;
    KlDgramLife *l = kl_dgram_life_create(&a, (void *)0x1, on_final, NULL, KL_DGRAM_OWNER_DATAGRAM, (KlDgramDispatchFn)0);
...
UTEST(dgram_life, idempotent_and_null_safe) {
    KlAllocator a = kl_allocator_default();
    g_final = 0;
    KlDgramLife *l = kl_dgram_life_create(&a, (void *)0x9, on_final, NULL, KL_DGRAM_OWNER_DATAGRAM, (KlDgramDispatchFn)0);
    kl_dgram_life_mark_dead(l);
