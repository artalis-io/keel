...
UTEST(chunked, single_chunk) {
    ...
    const char *input = "5\r\nhello\r\n0\r\n\r\n";
    int rc = kl_chunked_decode(&dec, input, strlen(input), &m.base);
    ASSERT_EQ(rc, 1);
...
UTEST(chunked, uppercase_hex) {
    ...
    int rc = kl_chunked_decode(&dec, input, 20, &m.base);
    ASSERT_EQ(rc, 1);
    ASSERT_EQ(m.len, (size_t)10);
}
...
UTEST(chunked, byte_at_a_time) {
    ...
    for (size_t i = 0; i < total; i++) {
        rc = kl_chunked_decode(&dec, input + i, 1, &m.base);
        if (rc != 0) break;
    }
    ASSERT_EQ(rc, 1);
...
UTEST(chunked, split_across_calls) {
    ...
    int rc = kl_chunked_decode(&dec, p1, strlen(p1), &m.base);
    ASSERT_EQ(rc, 0);

    rc = kl_chunked_decode(&dec, p2, strlen(p2), &m.base);
    ASSERT_EQ(rc, 1);
    ASSERT_EQ(m.len, (size_t)5);
    ASSERT_TRUE(memcmp(m.data, "hello", 5) == 0);
}
