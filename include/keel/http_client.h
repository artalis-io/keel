...
#ifndef KEEL_HTTP_CLIENT_H
#define KEEL_HTTP_CLIENT_H
...
#define KL_HTTP_CLIENT_HOSTNAME_MAX       256
/** @brief Request buffer size. */
#define KL_HTTP_CLIENT_REQ_BUF_SIZE       4096
/** @brief Maximum request headers. */
#define KL_HTTP_CLIENT_MAX_REQ_HEADERS    64
/** @brief Default timeout (ms). */
#define KL_HTTP_CLIENT_DEFAULT_TIMEOUT_MS 30000
/** @brief Default max response size. */
#define KL_HTTP_CLIENT_DEFAULT_MAX_RESP   (4 * 1024 * 1024)
/** @brief Receive buffer size. */
#define KL_HTTP_CLIENT_RECV_BUF_SIZE      8192
/** @brief Chunked encoding buffer size. */
#define KL_HTTP_CLIENT_CHUNK_BUF_SIZE     4096
/** @brief Chunked header size. */
#define KL_HTTP_CLIENT_CHUNK_HDR_SIZE     24    /**< Chunk-size line: up to 16 hex digits + "\r\n" + NUL (18+1); 24 = margin. Real chunks are <= chunk_buf so <= 3 hex digits. */
/** @brief Final chunk length. */
#define KL_HTTP_CLIENT_FINAL_CHUNK_LEN    5     /**< "0\\r\\n\\r\\n" */
/** @brief Default Happy Eyeballs Connection Attempt Delay, ms (RFC 8305 §5). */
#define KL_HTTP_CLIENT_CONNECT_ATTEMPT_DELAY_MS 250
...
typedef struct KlHttpClientResponse {
    int              status;
    char            *body;
    size_t           body_len;
 ...
 * @param user_data  User context from KlHttpClientStreamCfg.
 * @return 0 to continue, -1 to abort.
 */
typedef int (*KlHttpClientBodyFn)(const char *data, size_t len, void *user_data);

/**
 * @brief Called when response headers are complete.
 * @return 0 to continue, -1 to abort.
 */
typedef int (*KlHttpClientHeadersFn)(int status, const KlHttpClientHeader *headers,
 ...
 */
typedef kl_ssize_t (*KlHttpClientReadFn)(char *buf, size_t buf_len, void *user_data);

/**
 * @brief Per-request streaming configuration.
 *
 * When stream is non-NULL and on_body is set, response body chunks are
 * pushed via on_body instead of being accumulated into KlHttpClientResponse.body.
 *
 ...
 */
typedef struct {
    KlHttpClientBodyFn     on_body;        /**< Response body callback (NULL = buffer) */
    KlHttpClientHeadersFn  on_headers;    /**< NULL = skip */
    void             (*on_complete)(void *user_data);  /**< NULL = skip */

    KlHttpClientReadFn     body_read;     /**< Request body pull callback (NULL = use body/body_len) */

    void              *user_data;     /**< shared across all callbacks */
} KlHttpClientStreamCfg;
 ...
 *
 * Same as kl_http_client_request but supports response and request body streaming.
 * When stream is NULL, behaves identically to kl_http_client_request.
 *
 * @param alloc       Allocator for response data.
 * @param cfg         Client config (timeouts, TLS, limits). May be NULL for defaults.
 * @param method      HTTP method ("GET", "POST", etc.).
 ...
 * @param body        Request body (may be NULL).
 * @param body_len    Request body length.
 * @param stream      Streaming config (NULL = buffer mode, same as kl_http_client_request).
 * @param resp        Output: populated on success. Caller must call kl_http_client_response_free().
 * @return 0 on success, -1 on error.
 */
...
typedef struct KlHttpClient KlHttpClient;
 ...
 * The caller must then inspect the result and call kl_http_client_free().
 *
 ...
 *
 * Same as kl_http_client_start but supports response and request body streaming.
 * When stream is NULL, behaves identically to kl_http_client_start.
 *
 ...
 * @param body        Request body (may be NULL).
 * @param body_len    Request body length.
 * @param stream      Streaming config (NULL = buffer mode, same as kl_http_client_start).
 * @param on_done     Callback invoked on completion or error.
 * @param user_data   User data passed to on_done.
 * @return Client handle, or NULL on immediate failure.
 */
 ...
 * @return Response pointer (valid until kl_http_client_free), or NULL if error.
 */
 ...
 *
 * Safe to call after kl_http_client_cancel() or after completion callback.
 */
 ...
 * KlHttpClientResponse. Body is NULL and body_len is 0 in the response.
 *
